...
@dataclass(frozen=True)
class ShallowWaterTilemapImage:
    tilemap: object
    left: int
    top: int
    width: int
...
class Renderer:
    ...
    def __init__(self, pyxel_module, sprite_assets: SpriteAssetLibrary | None = None) -> None:
        ...
        self._shallow_water_tilemap_cache: dict[tuple[object, ...], ShallowWaterTilemapImage] = {}
    ...
    def draw_scene(
        ...
        if not self.draw_shallow_water_tilemap_layer(model, camera):
    ...
    def draw_shallow_water_tilemap_layer(self, model: GameModel, camera: CameraState) -> bool:
        config = model.config.get("shallow_water", {})
        if (
            not config.get("enabled", False)
            or not config.get("surface_tiles_enabled", False)
            or not config.get("surface_tilemap_enabled", False)
        ):
            return False
        if not self.camera_is_affine(camera):
            return False
        ...
        drew_any = False
        for area in areas:
            baked = self.shallow_water_tilemap_image(model, camera, area, config)
            if baked is None:
                return False
            if self.screen_rect_visible(
                ScreenRect(baked.left, baked.top, baked.width, baked.height),
                camera,
                2.0,
            ):
                self.pyxel.bltm(
    ...
    def shallow_water_tilemap_image(
    ...
    ) -> ShallowWaterTilemapImage | None:
        rect = area.rect
        tile_world_size = max(1.0, float(config.get("surface_tile_world_size", 64.0)))
        shoreline_tile_world_size = max(1.0, float(config.get("shoreline_tile_world_size", 64.0)))
        bake_margin = (
            shoreline_tile_world_size * 0.5 if config.get("shoreline_tiles_enabled", False) else 0.0
        )
        bounds = self.ground_source_screen_bounds(
            camera,
            rect.min_x - bake_margin,
            rect.min_z - bake_margin,
            rect.width + bake_margin * 2.0,
            rect.depth + bake_margin * 2.0,
        )
        if bounds is None:
            return None
        tile_cols = max(1, math.ceil(bounds.width / 8))
        tile_rows = max(1, math.ceil(bounds.height / 8))
        image_width = tile_cols * 8
        image_height = tile_rows * 8
        if image_width > 1024 or image_height > 1024:
            return None

        key = self.shallow_water_tilemap_cache_key(
            model,
            camera,
            area,
            config,
        ...
        cached = self._shallow_water_tilemap_cache.get(key)
        if cached is not None:
            return ShallowWaterTilemapImage(
                tilemap=cached.tilemap,
                left=bounds.x,
                top=bounds.y,
                width=cached.width,
        ...
        image = self.pyxel.Image(image_width, image_height)
        if hasattr(image, "cls"):
            image.cls(8)
        else:
        ...
        surface_pattern = self.shallow_water_surface_pattern(config)
        shoreline_tiles = self.shallow_water_shoreline_placements(model, area, config)

        for py in range(image_height):
            screen_y = bounds.y + py + 0.5
            for px in range(image_width):
                screen_x = bounds.x + px + 0.5
                ground = screen_to_ground_affine(camera, screen_x, screen_y)
                if ground is None:
                    continue
                color = self.shallow_water_color_at(
                    model,
                    rect,
                    surface_pattern,
                    tile_world_size,
        ...
        tilemap = self.pyxel.Tilemap(tile_cols, tile_rows, image)
        for ty in range(tile_rows):
            for tx in range(tile_cols):
                tilemap.pset(tx, ty, (tx, ty))

        baked = ShallowWaterTilemapImage(
            tilemap=tilemap,
            left=bounds.x,
            top=bounds.y,
            width=image_width,
        ...
        self._shallow_water_tilemap_cache[key] = baked
        return baked

    def shallow_water_tilemap_cache_key(
        self,
        model: GameModel,
        camera: CameraState,
        area,
        ...
        if not isinstance(tile_names, dict):
            tile_names = {}
        return (
            "shallow_water_tilemap_v1",
            area.id,
            area.rect.min_x,
            area.rect.min_z,
            area.rect.max_x,
            area.rect.max_z,
            round(float(getattr(camera, "effective_scale", 1.0)), 6),
            round(float(getattr(camera, "anchor_x", 0.0)), 6),
            round(float(getattr(camera, "anchor_y", 0.0)), 6),
            int(getattr(camera, "viewport_width", 0)),
            int(getattr(camera, "viewport_height", 0)),
            image_width,
            image_height,
            bounds.width,
            bounds.height,
            float(config.get("surface_tile_world_size", 64.0)),
            tuple(tuple(row) for row in config.get("surface_tile_pattern", ())),
            bool(config.get("shoreline_tiles_enabled", False)),
            float(config.get("shoreline_tile_world_size", 64.0)),
            bool(config.get("shoreline_corner_tiles_enabled", True)),
            float(config.get("shoreline_corner_inset_world", 0.0)),
            tuple(sorted(tile_names.items())),
            self.sprite_assets_revision_key(model, config),
        )

    def sprite_assets_revision_key(self, model: GameModel, config: dict) -> tuple[object, ...]:
        ids: set[str] = set()
        pattern = config.get("surface_tile_pattern", ())
        if isinstance(pattern, list):
            for row in pattern:
                if isinstance(row, list):
                    ids.update(str(item) for item in row if isinstance(item, str))
        tile_names = config.get("shoreline_tiles", {})
        if isinstance(tile_names, dict):
            ids.update(str(item) for item in tile_names.values() if isinstance(item, str))

        revisions: list[tuple[str, str, str]] = []
        for visual_id in sorted(ids):
            asset = self.ground_surface_sprite_asset(model, visual_id)
            if asset is None:
                revisions.append((visual_id, "", ""))
                continue
            frame = asset.frame()
            revisions.append((visual_id, asset.definition.asset_id, frame.source_hash or ""))
        return tuple(revisions)

    def shallow_water_surface_pattern(self, config: dict) -> tuple[tuple[str, ...], ...]:
        pattern = config.get("surface_tile_pattern", ())
        ...
        for row in pattern:
            if isinstance(row, list):
                ids = tuple(item for item in row if isinstance(item, str))
                if ids:
                    rows.append(ids)
        return tuple(rows)

    def shallow_water_shoreline_placements(
        self, model: GameModel, area, config: dict
    ) -> tuple[tuple[LoadedSpriteAsset, float, float], ...]:
        if not config.get("shoreline_tiles_enabled", False):
            return ()
        tile_names = config.get("shoreline_tiles", {})
        if not isinstance(tile_names, dict):
            return ()
        ...
        placements: list[tuple[LoadedSpriteAsset, float, float]] = []

        def add_tile(name_key: str, x: float, z: float) -> None:
            visual = tile_names.get(name_key)
            if not isinstance(visual, str):
                return
            asset = self.ground_surface_sprite_asset(model, visual)
            if asset is not None:
                placements.append((asset, x, z))

        columns = max(1, math.ceil(rect.width / tile_world_size))
        rows = max(1, math.ceil(rect.depth / tile_world_size))
        for column_index in range(columns):
            x = rect.min_x + tile_world_size * (column_index + 0.5)
            add_tile("top", x, rect.min_z)
            add_tile("bottom", x, rect.max_z)
        for row_index in range(rows):
            z = rect.min_z + tile_world_size * (row_index + 0.5)
            add_tile("left", rect.min_x, z)
            add_tile("right", rect.max_x, z)
        if draw_corners:
            add_tile("corner_nw", rect.min_x + corner_inset, rect.min_z + corner_inset)
            add_tile("corner_ne", rect.max_x - corner_inset, rect.min_z + corner_inset)
            add_tile("corner_sw", rect.min_x + corner_inset, rect.max_z - corner_inset)
            add_tile("corner_se", rect.max_x - corner_inset, rect.max_z - corner_inset)
        return tuple(placements)

    def shallow_water_color_at(
        self,
        model: GameModel,
        rect: WorldRect,
        surface_pattern: tuple[tuple[str, ...], ...],
        tile_world_size: float,
        shoreline_tiles: tuple[tuple[LoadedSpriteAsset, float, float], ...],
        x: float,
        z: float,
    ) -> int | None:
        color: int | None = None
        if rect.contains_point(x, z) and surface_pattern:
            local_x = max(0.0, min(rect.width - 1e-6, x - rect.min_x))
            local_z = max(0.0, min(rect.depth - 1e-6, z - rect.min_z))
            tile_col = int(local_x // tile_world_size)
            tile_row = int(local_z // tile_world_size)
            pattern_row = surface_pattern[tile_row % len(surface_pattern)]
            visual = pattern_row[tile_col % len(pattern_row)]
            asset = self.ground_surface_sprite_asset(model, visual)
            color = self.sample_ground_source_asset_color(
                asset,
                rect.min_x + tile_world_size * (tile_col + 0.5),
                rect.min_z + tile_world_size * (tile_row + 0.5),
                x,
                z,
            )

        for asset, tile_x, tile_z in shoreline_tiles:
            overlay = self.sample_ground_source_asset_color(asset, tile_x, tile_z, x, z)
            if overlay is not None:
                color = overlay
        return color

    def sample_ground_source_asset_color(
        self, asset: LoadedSpriteAsset | None, center_x: float, center_z: float, x: float, z: float
    ) -> int | None:
        ...
        origin_x = center_x - anchor_x * width_world / source.width
        origin_z = center_z - anchor_y * depth_world / source.height
        if (
            x < origin_x
            or z < origin_z
            or x >= origin_x + width_world
            or z >= origin_z + depth_world
        ):
            return None
        col_index = int((x - origin_x) / max(width_world, 1e-6) * source.width)
        row_index = int((z - origin_z) / max(depth_world, 1e-6) * source.height)
        if (
            col_index < 0
            or row_index < 0
            or col_index >= source.width
            or row_index >= source.height
        ):
            return None
        char = source.rows[row_index][col_index]
        if char == format(asset.definition.colkey, "X"):
            return None
        return int(char, 16)
