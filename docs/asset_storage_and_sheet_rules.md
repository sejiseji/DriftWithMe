...
配布済み基準アセットは `jack_idle_32` / `idle_00`。

|項目|値|
|---|---|
|画像バンク|0|
|source rect|`(u,v,w,h) = (0,0,32,32)`|
|colkey|0|
|anchor_px|`(16,32)`。画像下辺中央の境界座標|
|world_size|`(16.0,16.0)`|
|projection_mode|`upright_height_billboard_v1`|
|flip_policy|`none`|
|source_hash|`9e63b51c48c928394fc992bda83c1c851588f69ba4dae7be7e340054d885cd7d`|

Jackの画素、アンカー、表示寸法、hover、影、深度順、collider、入力、カメラは今回変更しない。

### 4.2 Jackの初期配置台帳案

以下は未制作フレームの配置予約案。現行pyxresとmetadataを確認した範囲では、現行Jackの実フレーム `(0,0,32,32)` とは競合しない。予約のみであり、今回書き込みや参照は行わない。

|行|用途|コマ0|コマ1|コマ2|コマ3|
|---|---|---|---|---|---|
|基準絵|現行 `idle_00`|`(0,0)`|未割当|未割当|未割当|
|front|正面系|`(0,32)`|`(32,32)`|`(64,32)`|`(96,32)`|
|left|左向き|`(0,64)`|`(32,64)`|`(64,64)`|`(96,64)`|
|right|右向き|`(0,96)`|`(32,96)`|`(64,96)`|`(96,96)`|
|back|背面系|`(0,128)`|`(32,128)`|`(64,128)`|`(96,128)`|
